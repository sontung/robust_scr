...
def main():
    ...
    g2 = torch.load("/home/n11373598/work/scrstudio/outputs/aachen/node2vec/2025-04-02_115205/scrstudio_models/head.pt")
    node2vec_desc = g2["model.embedding.weight"].cpu().numpy()
    desc2 = np.load(
    ...
    salad_desc = np.load("/home/n11373598/hpc-home/work/glace_experiment/checkpoints/desc_salad_db.npy")
    ...
    print(np.mean(s0), np.mean(s1))

    _, i2 = retrieve(salad_desc.astype(np.float32), top_k=2)
    s2 = compute_overlapping_scores(covis_score, i2)

    # false negatives
    mask = s2[:, 0] == 0
    print(i2[mask])
    print(np.mean(s0), np.mean(s1))
    path_indices = np.arange(len(s2))[mask]

    # ds.rgb_files = np.array(ds.rgb_files)
    # print(i2[mask][0], ds.rgb_files[i2[mask][0]], s2[mask][0])
    # print(i1[mask][0], ds.rgb_files[i1[mask][0]], s1[mask][0])
    # print(i0[mask][0], ds.rgb_files[i0[mask][0]], s0[mask][0])
    #
    # for ind1, ind2 in i2[mask]:
    #     n1 = ds.rgb_files[ind1]
    #     n2 = ds.rgb_files[ind2]
    #     img1 = Image.open(f"/home/n11373598/hpc-home/work/glace_experiment/datasets/aachen11/train/rgb/{n1}").convert("RGB")
    #     img2 = Image.open(f"/home/n11373598/hpc-home/work/glace_experiment/datasets/aachen11/train/rgb/{n2}").convert("RGB")
    #     img1 = img1.resize((1500, 1000), Image.BILINEAR)
    #     img2 = img2.resize((1500, 1000), Image.BILINEAR)
    #     combined = np.hstack((np.array(img1), np.array(img2)))
    #     cv2.imwrite(f"../debug/false_negative_{ind1}_{ind2}.jpg",
    #                 cv2.cvtColor(combined, cv2.COLOR_RGB2BGR))
    #
    #
    # o_scores = covis_score.tocoo()
    # mask = o_scores.data > 0.5
    # c1 = o_scores.row[mask]
    # c2 = o_scores.col[mask]
    # o_scores_vals = o_scores.data[mask]
    # dis = compute_errors(ds.pose_values, np.vstack([c1, c2]).T).ravel()
    # k = 10
    # topk_indices = np.argpartition(-dis, k - 1)[:k]
    # data_dir = "/home/n11373598/hpc-home/work/glace_experiment/datasets/aachen11/train/rgb"
    # for id0, ind in enumerate(topk_indices):
    #     n1 = ds.rgb_files[c1[ind]]
    #     n2 = ds.rgb_files[c2[ind]]
    #     score = o_scores_vals[ind]
    #     img1 = Image.open(f"{data_dir}/{n1}").convert("RGB")
    #     img2 = Image.open(f"{data_dir}/{n2}").convert("RGB")
    #     print(img1.size, img2.size, score)
    #     img1 = img1.resize((1500, 1000), Image.BILINEAR)
    #     img2 = img2.resize((1500, 1000), Image.BILINEAR)
    #     combined = np.hstack((np.array(img1), np.array(img2)))
    #     cv2.imwrite(f"../debug/covis_{id0}.jpg",
    #                 cv2.cvtColor(combined, cv2.COLOR_RGB2BGR))
    #
